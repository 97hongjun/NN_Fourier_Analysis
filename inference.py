...
def sample_ball(d, n):
	...
	X = np.divide(X, nrm)
	U = np.sqrt(d)*(np.random.uniform(0, 1, n)**(1/float(d)))
...
def compute_covariance_matrix_massive(Xs, r):
	m, d = Xs.shape
	m_div = int(m/r)+1
	...
	log_coeff = loggamma(2.0*alpha)-2.0*loggamma(alpha) - np.log(alpha) - (2.0*alpha-1.0)*np.log(2.0)
